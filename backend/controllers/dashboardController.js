    ...
    const globalStats = {
      ...
      mostReviewedVideo:
        videosWithStats.length > 0
          ? videosWithStats.reduce((prev, current) => (prev.reviewsCount > current.reviewsCount ? prev : current)).title
          : null,
      mostLikedVideo:
        videosWithStats.length > 0
          ? videosWithStats.reduce((prev, current) => (prev.likesCount > current.likesCount ? prev : current)).title
          : null,
    }
